...
static uint8_t build_dev_address(eeprom_handle_t *handle, uint16_t read_write_memory_address)
{
	...
	if (handle->addressing_type == EEPROM_ADDRESSING_TYPE_8BIT_1BIT_OVERFLOW) {
		address_overflow = ((read_write_memory_address >> 8) & 0x1) << 2;
	}
...
bool eeprom_write_byte(eeprom_handle_t *handle, uint16_t address, uint8_t value)
{
	...
	if (HAL_I2C_Mem_Write(handle->i2c_handle, build_dev_address(handle, address), address, get_mem_address_size(handle),
			&value, 1, EEPROM_I2C_TIMEOUT) != HAL_OK) {
		return false;
	}
...
bool eeprom_write_bytes(eeprom_handle_t *handle, uint16_t address, uint8_t *source, size_t count)
{
	...
	while (count > 0) {
		...
		if (HAL_I2C_Mem_Write(handle->i2c_handle, build_dev_address(handle, address), current_address,
				get_mem_address_size(handle), source_ptr, bytes_to_write, EEPROM_I2C_TIMEOUT) != HAL_OK) {
			return false;
		}
...
bool eeprom_read_bytes(eeprom_handle_t *handle, uint16_t address, uint8_t *dest, size_t count)
{
	...
	return HAL_I2C_Mem_Read(handle->i2c_handle, build_dev_address(handle, address), address,
